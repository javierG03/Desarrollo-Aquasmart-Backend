...
class PlotSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            Plot.objects.exclude(id_plot=instance.id_plot if instance else None)
            .filter(latitud=latitud, longitud=longitud)
            .exists()
        ):
            raise serializers.ValidationError(
                "La georeferenciación ya está asignada a otro predio."
            )
        return data
...
class LotSerializer(serializers.ModelSerializer):
    ...
    def validate_plot(self, value):
        ...
        if not value.is_activate:
            raise serializers.ValidationError(
                "El predio asociado se encuentra inactivo."
            )
        return value
...
class LotDetailSerializer(LotSerializer):
    ...
    plot_owner = serializers.CharField(
        source="plot.owner.get_full_name", read_only=True
    )
    soil_type_name = serializers.CharField(source="soil_type.name", read_only=True)

    class Meta(LotSerializer.Meta):
        fields = LotSerializer.Meta.fields + [
            "plot_name",
            "plot_owner",
            "soil_type_name",
        ]
