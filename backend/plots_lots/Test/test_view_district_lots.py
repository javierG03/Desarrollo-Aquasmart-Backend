...
@pytest.fixture
def registered_lots(db, admin_user, normal_user, registered_plot, soil_type, crop_type):
    """Crea varios lotes en la base de datos, algunos para el admin y otros para un usuario normal."""

    admin_lots = [
        Lot.objects.create(
            plot=registered_plot,
            crop_type=crop_type,  # ✅ Se usa la instancia
            soil_type=soil_type,
            is_activate=True,
        )
        for _ in range(3)
    ...
    # Si quieres que los del usuario tengan otro tipo de cultivo:
    user_crop_type = CropType.objects.get_or_create(name="Trigo")[0]

    normal_user_lots = [
        Lot.objects.create(
            plot=user_plot,
            crop_type=user_crop_type,  # ✅ Se usa la instancia
            soil_type=soil_type,
            is_activate=True,
        )
        for _ in range(3)
    ...
    return admin_lots + normal_user_lots
