...
class TestPlotStatus:
    ...
    def test_view_active_plot_details(self, authenticated_regular_client, active_plot):
        ...
        assert response.status_code == status.HTTP_200_OK, "Debería obtener el detalle del predio"
        assert response.data["is_activate"] is True, "El predio debería mostrarse como activo"
    
    def test_view_inactive_plot_details(self, authenticated_regular_client, inactive_plot):
        ...
        assert response.status_code == status.HTTP_200_OK, "Debería obtener el detalle del predio"
        assert response.data["is_activate"] is False, "El predio debería mostrarse como inactivo"
    
    def test_cannot_create_lot_in_inactive_plot(self, authenticated_regular_client, inactive_plot, soil_type):
        ...
        assert response.status_code in [status.HTTP_400_BAD_REQUEST, status.HTTP_403_FORBIDDEN], "Debería rechazar la creación del lote"
        
        # Si es 403, asumimos que está bien y no verificamos el mensaje (el sistema lo maneja como restricción de permisos)
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            assert "inactivo" in str(response.data).lower() or "inhabilitado" in str(response.data).lower()
    
    def test_cannot_update_inactive_plot(self, authenticated_regular_client, inactive_plot):
        ...
        assert "inactivo" in str(response.data).lower() or "inhabilitado" in str(response.data).lower() or "error" in str(response.data).lower()
...
class TestPlotActivation:
    ...
    def test_admin_can_inactivate_plot(self, authenticated_admin_client, active_plot):
        ...
        assert response.status_code == status.HTTP_200_OK, "La inhabilitación debería ser exitosa"
        ...
        assert active_plot.is_activate is False, "El campo is_activate debería ser False"
    ...
    def test_admin_can_activate_plot(self, authenticated_admin_client, inactive_plot):
        ...
        assert response.status_code == status.HTTP_200_OK, "La habilitación debería ser exitosa"
        assert "mensaje" in response.data, "Debería haber un mensaje de éxito"
        
        # Verificar que el predio realmente se habilitó
        inactive_plot.refresh_from_db()
        assert inactive_plot.is_activate is True, "El campo is_activate debería ser True"
    
    def test_regular_user_cannot_inactivate_plot(self, authenticated_regular_client, active_plot):
        """Verifica que un usuario regular no puede inhabilitar un predio."""
        url = reverse("inhabilitar-predio", kwargs={"id_plot": active_plot.id_plot})
        response = authenticated_regular_client.post(url)
        
        assert response.status_code == status.HTTP_403_FORBIDDEN, "Debería rechazar el acceso"
        
        # Verificar que el predio no cambió
        active_plot.refresh_from_db()
        assert active_plot.is_activate is True, "El predio debería seguir activo"
    
    def test_cannot_inactivate_already_inactive_plot(self, authenticated_admin_client, inactive_plot):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST, "Debería indicar un error"
        assert "ya está desactivado" in str(response.data).lower() or "error" in str(response.data).lower()
    
    def test_cannot_activate_already_active_plot(self, authenticated_admin_client, active_plot):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST, "Debería indicar un error"
        assert "ya está activado" in str(response.data).lower() or "error" in str(response.data).lower()
    ...
    def test_inactivate_response_time(self, authenticated_admin_client, active_plot):
        ...
        assert response_time < 5.0, f"Tiempo de respuesta demasiado alto: {response_time} segundos"
...
class TestPlotAlerts:
    ...
    def test_alert_for_disabled_plot_actions(self, authenticated_admin_client, inactive_plot, soil_type):
        """
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST, "Debería rechazar la creación del lote con 400"
        ...
        assert "inactivo" in response_content, "La respuesta debe indicar que el predio está inactivo"
    
    def test_confirmation_alert_when_deactivating_plot(self, authenticated_admin_client, active_plot):
        """
        ...
        if response.status_code == status.HTTP_405_METHOD_NOT_ALLOWED:
            ...
            has_confirmation = any(
                ...
                for term in ["inhabilitado", "desactivado", "éxito", "exitosamente", "realizado"]
            )
            
            assert has_confirmation, "La respuesta debe confirmar que el predio fue inhabilitado"
    
    def test_success_alert_after_deactivating_plot(self, authenticated_admin_client, active_plot):
        """
        ...
        assert has_success_message, f"La respuesta debe confirmar el éxito de la operación. Mensaje actual: {message}"
        
        # Verificar que el predio realmente se deshabilitó
        active_plot.refresh_from_db()
        assert active_plot.is_activate is False, "El predio debería estar inactivo después de la operación"
...
class TestPlotActivationAdvanced:
    ...
    def test_data_integrity_after_activation_changes(self, authenticated_admin_client, inactive_plot):
        ...
        assert Decimal(str(inactive_plot.latitud)).normalize() == original_latitud, "La latitud no debería cambiar"
        assert Decimal(str(inactive_plot.longitud)).normalize() == original_longitud, "La longitud no debería cambiar"
        assert Decimal(str(inactive_plot.plot_extension)).normalize() == original_extension, "La extensión no debería cambiar"
    ...
    def test_multiple_activation_changes(self, authenticated_admin_client, active_plot):
        ...
        # 1. Desactivar
        url_deactivate = reverse("inhabilitar-predio", kwargs={"id_plot": active_plot.id_plot})
        response1 = authenticated_admin_client.post(url_deactivate)
        assert response1.status_code == status.HTTP_200_OK
        
        active_plot.refresh_from_db()
        assert active_plot.is_activate is False, "Debería estar inactivo después de desactivar"
        
        # 2. Activar
        url_activate = reverse("habilitar-predio", kwargs={"id_plot": active_plot.id_plot})
        response2 = authenticated_admin_client.post(url_activate)
        assert response2.status_code == status.HTTP_200_OK
        
        active_plot.refresh_from_db()
        assert active_plot.is_activate is True, "Debería estar activo después de activar"
        ...
        active_plot.refresh_from_db()
        assert active_plot.is_activate is False, "Debería estar inactivo después de desactivar de nuevo"
    
    def test_related_lots_behavior(self, authenticated_admin_client, active_plot, soil_type):
    ...
    def test_complete_flow_with_admin(self, api_client, admin_user, active_plot):
        ...
        deactivate_url = reverse("inhabilitar-predio", kwargs={"id_plot": active_plot.id_plot})
