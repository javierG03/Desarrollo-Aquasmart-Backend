...
def test_register_plot_success(api_client, admin_user):
    ...
    assert response.status_code == status.HTTP_201_CREATED, f"Error en el registro del predio: {response.data}"
    assert "id_plot" in response.data, "❌ No se recibió el identificador del predio."
    assert response.data["id_plot"] == response.data["id_plot"], "❌ El ID del predio no coincide."

    print("✅ Test completado con éxito. El administrador pudo registrar un predio correctamente.")
...
def test_register_plot_missing_fields(api_client, admin_user):
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST, "❌ La API permitió registrar un predio con datos incompletos."
    print("✅ Test completado con éxito: No se permitió registrar un predio con datos faltantes.")
...
def test_register_plot_without_authentication(api_client):
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZED, "❌ La API permitió registrar un predio sin autenticación."
...
def test_register_plot_unauthorized_user(api_client, normal_user, otp_for_user):
    ...
    print("✅ Test completado con éxito. Un usuario sin permisos NO pudo registrar un predio.")
