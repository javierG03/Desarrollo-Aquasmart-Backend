...
class TestLotStatus:
    ...
    def test_view_active_lot_details(self, authenticated_regular_client, active_lot):
        ...
        assert response.status_code == status.HTTP_200_OK, "Debería obtener el detalle del lote"
        assert response.data["is_activate"] is True, "El lote debería mostrarse como activo"
    
    def test_view_inactive_lot_details(self, authenticated_regular_client, inactive_lot):
        ...
        assert response.status_code == status.HTTP_200_OK, "Debería obtener el detalle del lote"
        assert response.data["is_activate"] is False, "El lote debería mostrarse como inactivo"
    
    def test_cannot_update_inactive_lot(self, authenticated_regular_client, inactive_lot):
    ...
    def test_alert_on_inactive_lot_actions(self, authenticated_admin_client, inactive_lot, soil_type):
        """
...
class TestLotActivation:
    ...
    def test_regular_user_cannot_inactivate_lot(self, authenticated_regular_client, active_lot):
    ...
    def test_cannot_inactivate_already_inactive_lot(self, authenticated_admin_client, inactive_lot):
    ...
    def test_cannot_activate_already_active_lot(self, authenticated_admin_client, active_lot):
...
class TestLotAlerts:
    ...
    def test_alert_for_disabled_lot_actions(self, authenticated_admin_client, inactive_lot):
        """
        Verifica que se muestra una alerta al intentar realizar acciones en un lote inhabilitado.
    ...
    def test_confirmation_alert_when_deactivating_lot(self, authenticated_admin_client, active_lot):
        ...
        if response.status_code == status.HTTP_405_METHOD_NOT_ALLOWED:
            ...
            assert has_confirmation, "La respuesta debe confirmar que el lote fue inhabilitado"
    
    def test_success_alert_after_deactivating_lot(self, authenticated_admin_client, active_lot):
        """
        ...
        # Verificar que el lote realmente se deshabilitó
        active_lot.refresh_from_db()
        assert active_lot.is_activate is False, "El lote debería estar inactivo después de la operación"
...
class TestLotActivationAdvanced:
    ...
    def test_data_integrity_after_activation_changes(self, authenticated_admin_client, inactive_lot):
        ...
        assert inactive_lot.crop_type == original_crop_type, "El tipo de cultivo no debería cambiar"
        assert inactive_lot.crop_variety == original_crop_variety, "La variedad de cultivo no debería cambiar"
        assert inactive_lot.soil_type == original_soil_type, "El tipo de suelo no debería cambiar"
    ...
    def test_multiple_activation_changes(self, authenticated_admin_client, active_lot):
        ...
        active_lot.refresh_from_db()
        assert active_lot.is_activate is False, "Debería estar inactivo después de desactivar"
        ...
        active_lot.refresh_from_db()
        assert active_lot.is_activate is False, "Debería estar inactivo después de desactivar de nuevo"
