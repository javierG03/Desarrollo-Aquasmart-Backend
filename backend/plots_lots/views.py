...
class PlotViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if not has_changes:
            raise Exception(
                "No se detectaron cambios en los datos del predio, modifique al menos un campo para actualizar."
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"success": "Actualización exitosa.", "data": response.data},
            ...
            return Response(
                {
                    "error": "Error en la actualización del predio, por favor intente más tarde."
                },
    ...
    def inactive(self, request, pk=None):
        ...
        try:
            ...
            if not predio:
                return Response(
                    {"error": "Predio no encontrado."}, status=status.HTTP_404_NOT_FOUND
                )

            if not predio.is_activate:
                return Response(
                    {"error": "El predio ya está inhabilitado."},
            ...
            predio.save()
            return Response(
                {"success": "Predio inhabilitado exitosamente."},
                status=status.HTTP_200_OK,
            )

        except Exception as e:
            return Response(
                {"error": f"No se pudo inhabilitar el predio: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def active(self, request, pk=None):
        ...
        try:
            ...
            if predio.is_activate:
                return Response(
                    {"error": "El predio ya está habilitado."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            predio.save()
            return Response(
                {"success": "Predio habilitado exitosamente."},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
                {"error": "No se pudo habilitar el predio."},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
