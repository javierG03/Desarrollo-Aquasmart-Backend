...
class Lot(models.Model):
    plot = models.ForeignKey(
        Plot, on_delete=models.CASCADE, related_name="lotes", verbose_name="Predio"
    )
    id_lot = models.CharField(
        primary_key=True,
        max_length=15,
        unique=True,
        editable=False,
        verbose_name="ID de lote",
    )  # Campo para almacenar el ID único
    crop_type = models.CharField(
        max_length=20, null=False, blank=False, verbose_name="Tipo de cultivo"
    )
    crop_variety = models.CharField(
        max_length=20, null=True, blank=True, verbose_name="Variedad del cultivo"
    )
    soil_type = models.ForeignKey(
        SoilType, on_delete=models.CASCADE, verbose_name="Tipo de suelo"
    )
    is_activate = models.BooleanField(
        default=True,
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id_lot:
            ...
            self.id_lot = (
                f"{id_plot_sin_prefijo}-{numero_formateado}"  # Resultado: "1234567-001"
            )
