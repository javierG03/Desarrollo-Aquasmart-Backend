...
def test_logout(api_client, test_user):
    ...
    if token:
        ...
        assert logout_response.status_code == status.HTTP_200_OK, f"Error en logout: {logout_response.data}"

        # 🔹 Intentar acceder a un endpoint protegido con el token (debe fallar)
        protected_url = reverse("protected-endpoint")  # 🔥 Reemplaza con un endpoint real
        protected_response = api_client.get(protected_url, **headers)
        assert protected_response.status_code == status.HTTP_401_UNAUTHORIZED, "❌ Token sigue activo tras logout"

    else:
        # 🔹 Si no hay token, verificar que logout falle con error
        logout_response = api_client.post(logout_url)
        assert logout_response.status_code == status.HTTP_401_UNAUTHORIZED, f"❌ Logout debería fallar sin token: {logout_response.data}"
        print("⚠️ No se puede probar logout porque la API no genera un token tras validar OTP.")
