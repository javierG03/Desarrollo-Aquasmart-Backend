...
class TestUserProfileSerializer:
    ...
    def test_serializer_contains_expected_fields(self, test_user):
        ...
        assert set(serializer.data.keys()) == set(expected_fields), "El serializador no contiene todos los campos esperados"
...
class TestCustomUserModel:
    ...
    def test_custom_user_model_fields(self, test_user):
        ...
        assert hasattr(test_user, 'first_name'), "El modelo no tiene el campo 'first_name'"
        assert hasattr(test_user, 'last_name'), "El modelo no tiene el campo 'last_name'"
        ...
        assert hasattr(test_user, 'document_type'), "El modelo no tiene el campo 'document_type'"
        assert hasattr(test_user, 'person_type'), "El modelo no tiene el campo 'person_type'"
        assert hasattr(test_user, 'is_active'), "El modelo no tiene el campo 'is_active'"
        assert hasattr(test_user, 'is_registered'), "El modelo no tiene el campo 'is_registered'"
        assert hasattr(test_user, 'drive_folder_id'), "El modelo no tiene el campo 'drive_folder_id'"
    
    def test_custom_user_str_representation(self, test_user):
        """✅ Verifica la representación en string del modelo."""
        expected_str = f"{test_user.document} - {test_user.first_name} {test_user.last_name}"
        assert str(test_user) == expected_str, "La representación en string no es la esperada"
...
class TestUserProfilePermissions:
    ...
    def test_profile_endpoint_requires_authentication(self, api_client):
        ...
        assert response.status_code == status.HTTP_401_UNAUTHORIZED, "El endpoint no está requiriendo autenticación correctamente"
        assert 'detail' in response.data, "La respuesta de error no contiene el campo 'detail'"
        assert response.data['detail'] == 'Las credenciales de autenticación no se proveyeron.', "El mensaje de error es incorrecto"
...
def test_complete_login_flow_access_profile(api_client, test_user):
    ...
    assert profile_response.status_code == status.HTTP_200_OK, f"Error al acceder al perfil: {profile_response.data}"
...
def test_profile_endpoint_with_authenticated_user(api_client, test_user, auth_token):
    ...
    assert response.status_code == status.HTTP_200_OK, f"Error al acceder al perfil: {response.data}"
...
def test_profile_endpoint_response_structure(api_client, test_user, auth_token):
    ...
    for field in expected_fields:
        assert field in response.data, f"El campo '{field}' no está presente en la respuesta"
...
def test_error_alert_handling(api_client, monkeypatch):
    ...
    def mock_db_error(*args, **kwargs):
        raise OperationalError("Error de comunicación con la base de datos: connection timed out")
...
def test_profile_endpoint_with_valid_token(api_client, test_user, auth_token):
    ...
    assert response.status_code == status.HTTP_200_OK, f"Error con token válido: {response.data}"
    assert response.data['document'] == test_user.document
...
def test_profile_endpoint_with_invalid_token(api_client):
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZED, "No se está rechazando correctamente el token inválido"
    assert 'detail' in response.data, "La respuesta de error no contiene el campo 'detail'"
...
def test_profile_endpoint_without_token(api_client):
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZED, "No se está requiriendo correctamente la autenticación"
    assert 'detail' in response.data, "La respuesta de error no contiene el campo 'detail'"
    assert response.data['detail'] == 'Las credenciales de autenticación no se proveyeron.', "El mensaje de error es incorrecto"
...
def test_profile_contains_all_required_fields(api_client, test_user, auth_token):
    ...
    for field in required_fields:
        assert field in response.data, f"Campo requerido '{field}' no está presente en la respuesta"
...
def test_profile_api_response_time(api_client, test_user, auth_token):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response_time < 3.0, f"El tiempo de respuesta ({response_time:.2f}s) excede el límite de 3 segundos"
