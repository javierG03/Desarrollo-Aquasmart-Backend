...
class TestUserValidationAndStatusManagement:
    ...
    def test_disable_active_user(self, api_client, admin_user, registered_user):
        ...
        assert login_response.status_code == status.HTTP_200_OK, "Fallo en el inicio de sesión del administrador"
        ...
        assert otp_response.status_code == status.HTTP_200_OK, "Fallo en la validación del OTP"

        # Deshabilitar usuario
        disable_user_url = reverse("Inative-user", kwargs={"document": registered_user.document})
        response = api_client.patch(
            disable_user_url, 
            HTTP_AUTHORIZATION=f"Token {otp_response.data['token']}"
        )

        # Verificaciones
        assert response.status_code == status.HTTP_200_OK, f"Error al deshabilitar usuario: {response.data}"
        registered_user.refresh_from_db()
        assert registered_user.is_active is False, "El usuario no fue deshabilitado correctamente"
        assert "desactivado correctamente" in response.data['status'], "Mensaje de desactivación incorrecto"
    ...
    def test_activate_inactive_user(self, api_client, admin_user, registered_user):
        ...
        # Habilitar usuario
        activate_user_url = reverse("Activate-user", kwargs={"document": registered_user.document})
        response = api_client.patch(
            activate_user_url,
            HTTP_AUTHORIZATION=f"Token {otp_response.data['token']}"
        )
        ...
        # Verificaciones
        assert response.status_code == status.HTTP_200_OK, f"Error al activar usuario: {response.data}"
        registered_user.refresh_from_db()
        assert registered_user.is_active is True, "El usuario no fue activado correctamente"
        assert "a sido activado con exito" in response.data['status'], "Mensaje de activación incorrecto"
    ...
    def test_login_attempt_with_disabled_user(self, api_client, registered_user):
        ...
        login_url = reverse("login")
        login_data = {
            "document": registered_user.document, 
            "password": "UserPass123@"
        }
        response = api_client.post(login_url, login_data)

        # Verificaciones
        assert response.status_code == status.HTTP_403_FORBIDDEN, "No se rechazó el inicio de sesión para usuario inhabilitado"
        assert "inactiva" in response.data['error']['detail'], "Mensaje de error incorrecto para usuario inhabilitado"
    ...
    def test_multiple_failed_login_attempts(self, api_client, registered_user):
        ...
        # Verificar bloqueo
        assert response.status_code == status.HTTP_400_BAD_REQUEST, "No se bloqueó al usuario después de intentos fallidos"
        assert "Usuario bloqueado por 30 minutos" in str(response.data['error']['detail'][0]), "Mensaje de bloqueo incorrecto"

    @pytest.mark.django_db
    def test_unauthorized_user_cannot_change_user_status(self, api_client, registered_user):
        """
        ...
        login_url = reverse("login")
        login_data = {"document": unauthorized_user.document, "password": "UnauthorizedPass123@"}
        login_response = api_client.post(login_url, login_data)
        ...
        assert login_response.status_code == status.HTTP_200_OK, "Fallo en el inicio de sesión del usuario sin permisos"
        ...
        # Verificaciones
        assert response.status_code == status.HTTP_403_FORBIDDEN, "Se permitió cambiar el estado de usuario sin permisos"