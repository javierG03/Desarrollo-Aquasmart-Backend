...
def restricted_user(db):
    ...
    LoginRestriction.objects.create(user=user, attempts=5, blocked_until=now() + timedelta(minutes=30))
    return user
...
def temporarily_blocked_user(db):
    ...
    LoginRestriction.objects.create(user=user, attempts=4, blocked_until=None)  # Aún no bloqueado
    return user
...
def test_login_attempts_exceeded(api_client, test_user):
    ...
    for _ in range(5):  # 5 intentos fallidos
        response = api_client.post(url, {"document": test_user.document, "password": "WrongPass"})
...
def test_login_while_temporarily_blocked(api_client, temporarily_blocked_user):
    ...
    url = reverse("login")
    response = api_client.post(url, {"document": temporarily_blocked_user.document, "password": "WrongPass"})

    assert response.status_code == status.HTTP_400_BAD_REQUEST  # No está bloqueado aún, pero sigue fallando
...
def test_login_while_temporarily_blocked(api_client, temporarily_blocked_user):
    ...
    assert login_restriction.is_blocked(), "El usuario debería estar bloqueado tras el 5° intento."

    # Verificamos que `attempts` se haya reiniciado a 0 tras el bloqueo
    assert login_restriction.attempts == 0, f"Se esperaba 0 intentos tras el bloqueo, pero se encontraron {login_restriction.attempts}"

    # Verificar si el mensaje de bloqueo es el esperado
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "Usuario bloqueado por 30 minutos." in response.data["error"]["detail"][0], f"Mensaje inesperado: {response.data['error']['detail'][0]}"

@pytest.mark.django_db
def test_login_while_blocked(api_client, restricted_user):
    """❌ Usuario bloqueado aún no puede iniciar sesión."""
    url = reverse("login")
    response = api_client.post(url, {"document": restricted_user.document, "password": "BlockedPass123"})
