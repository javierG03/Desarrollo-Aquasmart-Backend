...
class LoginHistoryMigrationTest(TestCase):
    ...
    def test_login_creates_log_entry(self):
        ...
        response = self.client.post(
            reverse("login"),
            {"document": "123456789", "password": "testpass123"},
            format="json",
        )
        ...
        # Validar el OTP
        response = self.client.post(
            reverse("validate-otp"),
            {"document": "123456789", "otp": otp.otp},
            format="json",
        )
    ...
    def test_old_login_history_migration(self):
        ...
        old_entries = LogEntry.objects.filter(
            content_type=content_type, action=0  # CREATE = login en nuestra migración
        )
