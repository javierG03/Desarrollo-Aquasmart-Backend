...
from .views import (
    ...
    GroupedPermissionsView,
    UserPermissionsView,
    AddUserPermissionsView,
    RemoveUserPermissionsView,
...
urlpatterns = [
    ...
    path(
        "groups/<int:pk>/assign_permissions",
        GroupViewSet.as_view({"post": "assign_permissions"}),
        name="assign-permissions",
    ),
    # Endpoint para quitar permisos de un grupo
    path(
        "groups/<int:pk>/remove_permissions",
        GroupViewSet.as_view({"post": "remove_permissions"}),
    ...
    path(
        "grouped_permissions",
        GroupedPermissionsView.as_view(),
        name="grouped-permissions",
    ),
    # Usuarios
    ...
    path(
        "users/<int:user_id>/add_permissions",
        AddUserPermissionsView.as_view(),
        name="add-user-permissions",
    ),
    # Remover permisos de un usuario
    path(
        "users/<int:user_id>/remove_permission",
        RemoveUserPermissionsView.as_view(),
        name="remove-user-permissions",
    ),
    # Endpoint para darle un grupo(rol) a un usuario
    path(
        "users/<int:user_id>/assign_group",
        AssignGroupToUserView.as_view(),
        name="assign-group-to-user",
    ),
    # Endpoint para remover un grupo(rol) a un usuario
    path(
        "users/<int:user_id>/remove_group",
        RemoveGroupFromUserView.as_view(),
