...
private_key = (
    ...
    or os.environ.get(  # Intenta con os.getenv (local)
        "PRIVATE_KEY", ""
    )  # Intenta con os.environ.get (producción)
).replace("\\n", "\n")
...
def get_drive_service():
    ...
    credentials_info = {
        ...
        "private_key_id": os.environ.get(
            "PRIVATE_KEY_ID", default=os.getenv("PRIVATE_KEY_ID")
        ),  # Si tienes este valor
        "private_key": private_key,  # Formatea correctamente el private key
        "client_email": os.environ.get(
            "CLIENT_EMAIL", default=os.getenv("CLIENT_EMAIL")
        ),
        ...
        "auth_provider_x509_cert_url": os.environ.get(
            "AUTH_PROVIDER_CERT_URL", default=os.getenv("AUTH_PROVIDER_CERT_URL")
        ),
        "client_x509_cert_url": os.environ.get(
            "CLIENT_X509_CERT_URL", default=os.getenv("CLIENT_X509_CERT_URL")
        ),  # Si tienes este valor
...
def upload_file_to_drive(file, file_name, folder_id=None):
    ...
    service = (
        get_drive_service()
    )  # Asegúrate de que esta función esté bien definida para la autenticación
...
def create_folder(folder_name, parent_folder_id=None):
    ...
    if parent_folder_id:
        folder_metadata["parents"] = [
            parent_folder_id
        ]  # Crear dentro de una carpeta existente
