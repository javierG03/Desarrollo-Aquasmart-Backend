...
class IoTDeviceSerializer(serializers.ModelSerializer):
    ...
    id_lot = serializers.PrimaryKeyRelatedField(
        queryset=Lot.objects.all(), allow_null=True, required=False
    )
    owner_name = serializers.CharField(
        required=False, allow_blank=True
    )  # ✅ Permite que se envíe en la petición
    ...
    def validate(self, data):
        ...
        if id_plot and id_plot.owner and owner_name:
            raise serializers.ValidationError(
                {
                    "owner_name": "El propietario ya se obtiene del predio y no debe enviarse manualmente."
                }
            )
        ...
        if (
            id_lot
            and IoTDevice.objects.filter(
                id_lot=id_lot, device_type=device_type
            ).exists()
        ):
            raise serializers.ValidationError(
                {"device_type": f"Ya existe un dispositivo {device_type} en este lote."}
            )
