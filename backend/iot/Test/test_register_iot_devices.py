...
def test_admin_can_register_iot_device(api_client, admin_user, device_type):
    ...
    assert response.status_code == status.HTTP_201_CREATED, f"Error en el registro del dispositivo IoT: {response.data}"
    assert "iot_id" in response.data, "❌ No se recibió el identificador del dispositivo IoT."

    # 🔹 Confirmar que el dispositivo fue guardado en la base de datos
    iot_id = response.data["iot_id"]
    assert IoTDevice.objects.filter(iot_id=iot_id).exists(), "❌ El dispositivo IoT no fue guardado en la base de datos."
