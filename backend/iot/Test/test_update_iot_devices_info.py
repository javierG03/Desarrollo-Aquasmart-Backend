...
def test_admin_can_update_iot_device(api_client, admin_user, admin_devices):
    ...
    assert response.status_code == status.HTTP_200_OK, f"Error al actualizar el dispositivo: {response.data}"
...
@pytest.mark.django_db
def test_normal_user_cannot_update_other_users_iot_device(api_client, normal_user, admin_devices):
    ...
    iot_device = admin_devices[0]
    update_url = reverse("update_iot_device", kwargs={"iot_id": iot_device.iot_id})  # 🔥 Verifica la URL correcta
    ...
    assert response.status_code == status.HTTP_403_FORBIDDEN, f"❌ Un usuario normal pudo modificar un dispositivo de otro usuario. Respuesta: {response.data}"

    print("✅ Test completado: Un usuario normal NO puede actualizar dispositivos ajenos.")
