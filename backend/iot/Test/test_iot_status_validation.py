...
def test_deactivate_active_iot_device(api_client, admin_user, active_admin_iot_device):
    ...
    deactivate_url = reverse("deactivate_iot_device", kwargs={"iot_id": active_admin_iot_device.iot_id})
    response = api_client.patch(deactivate_url, **headers)
    
    # 🔹 Verificar respuesta exitosa
    assert response.status_code == status.HTTP_200_OK, f"Error al desactivar dispositivo: {response.data}"
    assert "message" in response.data, "❌ No se recibió mensaje de confirmación."
    assert "desactivado exitosamente" in response.data["message"], f"Mensaje inesperado: {response.data['message']}"
    
    # 🔹 Verificar que el dispositivo fue desactivado en la base de datos
    active_admin_iot_device.refresh_from_db()
    assert not active_admin_iot_device.is_active, "❌ El dispositivo no fue desactivado correctamente en la base de datos."
    
    print("✅ Test completado: Un administrador puede desactivar correctamente un dispositivo activo.")

@pytest.mark.django_db
def test_deactivate_already_inactive_iot_device(api_client, admin_user, inactive_admin_iot_device):
    ...
    deactivate_url = reverse("deactivate_iot_device", kwargs={"iot_id": inactive_admin_iot_device.iot_id})
    response = api_client.patch(deactivate_url, **headers)
    ...
    assert "ya está desactivado" in response.data["message"], f"Mensaje inesperado: {response.data['message']}"
    
    # 🔹 Verificar que el dispositivo sigue inactivo
    inactive_admin_iot_device.refresh_from_db()
    assert not inactive_admin_iot_device.is_active, "❌ El estado del dispositivo cambió inesperadamente."
    
    print("✅ Test completado: Al intentar desactivar un dispositivo ya inactivo se recibe un mensaje informativo.")

@pytest.mark.django_db
def test_user_can_deactivate_own_device(api_client, normal_user, active_user_iot_device):
    ...
    deactivate_url = reverse("deactivate_iot_device", kwargs={"iot_id": active_user_iot_device.iot_id})
    response = api_client.patch(deactivate_url, **headers)
    ...
    assert not active_user_iot_device.is_active, "❌ El dispositivo no fue desactivado correctamente en la base de datos."
    
    print("✅ Test completado: Un usuario normal puede desactivar sus propios dispositivos IoT.")

@pytest.mark.django_db
def test_user_cannot_deactivate_others_device(api_client, normal_user, active_admin_iot_device):
    ...
    if response.status_code == status.HTTP_403_FORBIDDEN:
        assert True, "✅ El sistema correctamente impide que usuarios normales modifiquen dispositivos ajenos."
    else:
        # Esta es una vulnerabilidad que debe reportarse y corregirse
        print("⚠️ VULNERABILIDAD DETECTADA: Un usuario normal puede desactivar dispositivos de otros usuarios.")
        ...
        if not active_admin_iot_device.is_active:
            print("❌ El dispositivo ajeno fue modificado por un usuario sin privilegios.")
        else:
    ...
    if response.status_code == status.HTTP_200_OK:
        print("❌ FALLO DE SEGURIDAD: El sistema permite a usuarios normales desactivar dispositivos ajenos")
        print("👨‍💻 Se recomienda implementar verificación de propiedad en 'DeactivateIoTDevice.patch()'")

@pytest.mark.django_db
def test_unauthenticated_user_cannot_deactivate_device(api_client, active_admin_iot_device):
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZED, f"❌ Un usuario no autenticado pudo desactivar un dispositivo: {response.data}"
    
    # 🔹 Verificar que el dispositivo sigue activo
    active_admin_iot_device.refresh_from_db()
    assert active_admin_iot_device.is_active, "❌ El estado del dispositivo cambió inesperadamente."
    
    print("✅ Test completado: Un usuario no autenticado NO puede desactivar dispositivos IoT.")

@pytest.mark.django_db
def test_activate_inactive_iot_device(api_client, admin_user, inactive_admin_iot_device):
    ...
    activate_url = reverse("activate_iot_device", kwargs={"iot_id": inactive_admin_iot_device.iot_id})
    response = api_client.patch(activate_url, **headers)
    
    # 🔹 Verificar respuesta exitosa
    assert response.status_code == status.HTTP_200_OK, f"Error al activar dispositivo: {response.data}"
    assert "message" in response.data, "❌ No se recibió mensaje de confirmación."
    assert "activado exitosamente" in response.data["message"], f"Mensaje inesperado: {response.data['message']}"
    
    # 🔹 Verificar que el dispositivo fue activado en la base de datos
    inactive_admin_iot_device.refresh_from_db()
    assert inactive_admin_iot_device.is_active, "❌ El dispositivo no fue activado correctamente en la base de datos."
    
    print("✅ Test completado: Un dispositivo inactivo puede ser activado correctamente.")

@pytest.mark.django_db
def test_activate_already_active_iot_device(api_client, admin_user, active_admin_iot_device):
    ...
    activate_url = reverse("activate_iot_device", kwargs={"iot_id": active_admin_iot_device.iot_id})
    response = api_client.patch(activate_url, **headers)
    
    # 🔹 Verificar respuesta informativa (no error)
    assert response.status_code == status.HTTP_200_OK, f"Error inesperado: {response.data}"
    assert "message" in response.data, "❌ No se recibió mensaje de información."
    assert "ya está activado" in response.data["message"], f"Mensaje inesperado: {response.data['message']}"
    
    # 🔹 Verificar que el dispositivo sigue activo
    active_admin_iot_device.refresh_from_db()
    assert active_admin_iot_device.is_active, "❌ El estado del dispositivo cambió inesperadamente."
    
    print("✅ Test completado: Al intentar activar un dispositivo ya activo se recibe un mensaje informativo.")