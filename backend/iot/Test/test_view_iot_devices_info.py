...
def test_normal_user_can_only_view_own_devices(api_client, normal_user, user_devices, admin_devices):
    ...
    user_devices_db = IoTDevice.objects.filter(id_plot__owner=normal_user)  # ✅ CORRECCIÓN: Usar id_plot__owner
    api_devices = [device for device in response.data if device["id_plot"] in list(Plot.objects.filter(owner=normal_user).values_list("id_plot", flat=True))]

    print("🔹 Dispositivos en la BD con dueño:", list(IoTDevice.objects.values("iot_id", "id_plot__owner")))
